...
- Combining `--person` with Year in Review creates a "Your Year With Alice" style video: only clips containing that person, but still with monthly structure. Naming two people narrows it to the clips holding both unless the CLI explicitly uses `--person-match or`.
