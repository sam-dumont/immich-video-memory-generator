...
def test_source_preparation_records_on_the_callers_existing_trace() -> None:
    shared_trace = Trace(warnings=["pool policy already ran"])

    prepared = prepare_editorial_source(
        EditorialSelectionRequest(scope=SourceScope()),
        EditorialDependencies(source_fetcher=lambda _scope: (make_asset("asset-1"),)),
        trace=shared_trace,
    )

    assert prepared.trace is shared_trace
    assert prepared.trace.warnings == ["pool policy already ran"]
    assert [item.name for item in prepared.trace.editorial_passes] == ["source-eligibility"]
...
def test_a_scoped_asset_inherits_its_full_corpus_episode() -> None:
    """A request projects onto canonical episodes instead of regrouping its subset."""
    ...
    prepared = prepare_editorial_source(
        ...
        EditorialDependencies(
            source_fetcher=lambda _scope: (
                make_asset("episode-context", file_created_at=noon),
                make_asset("in-scope", file_created_at=noon + timedelta(minutes=5)),
                make_asset("other-episode", file_created_at=noon + timedelta(hours=3)),
            )
        ),
    )
    ...
    assert tuple(
        (
            ...
            projection.group.candidate_ids,
            projection.scoped_candidate_ids,
        )
        for projection in projections
    ) == (
        (
            prepared.episode_groups[0].group_id,
            ("episode-context", "in-scope"),
            ("in-scope",),
        ),
    )


def test_a_scoped_asset_inherits_its_full_corpus_moment() -> None:
    """Cards keep the complete canonical moment while marking selectable members."""
    from immich_memories.analysis.selection_source_groups import project_moment_groups
    ...
    prepared = prepare_editorial_source(
        ...
        EditorialDependencies(
            source_fetcher=lambda _scope: (
                make_asset("moment-context", file_created_at=noon),
                make_asset("in-scope", file_created_at=noon + timedelta(minutes=5)),
                make_asset("later-moment", file_created_at=noon + timedelta(hours=3)),
            )
        ),
    )

    projections = project_moment_groups(prepared, ("in-scope",))

    assert tuple(
        (projection.group.candidate_ids, projection.scoped_candidate_ids)
        for projection in projections
    ) == ((("moment-context", "in-scope"), ("in-scope",)),)


@pytest.mark.parametrize("projection_kind", ("episode", "moment"))
def test_a_scope_cannot_escape_the_prepared_corpus(projection_kind: str) -> None:
    """A caller bug must not silently disappear as an empty projection."""
    from immich_memories.analysis.selection_source_groups import (
    ...
    prepared = prepare_editorial_source(
        EditorialSelectionRequest(scope=SourceScope()),
        EditorialDependencies(source_fetcher=lambda _scope: (make_asset("prepared"),)),
    )
    projector = {
        "episode": project_episode_groups,
        "moment": project_moment_groups,
    }[projection_kind]

    with pytest.raises(ValueError, match="scoped asset IDs must belong to prepared corpus"):
        projector(prepared, ("prepared", "escaped"))
...
def test_disjoint_date_windows_never_admit_the_gap_between_them() -> None:
    first_day = datetime(2020, 1, 1, 12, tzinfo=UTC)
    gap_day = datetime(2023, 1, 1, 12, tzinfo=UTC)
    last_day = datetime(2026, 1, 1, 12, tzinfo=UTC)
    windows = (
        DateRange(first_day, first_day),
        DateRange(last_day, last_day),
    )
    seen_scopes: list[SourceScope] = []

    def fetch(scope: SourceScope):
        seen_scopes.append(scope)
        return (
            make_asset("first", file_created_at=first_day),
            make_asset("gap", file_created_at=gap_day),
            make_asset("last", file_created_at=last_day),
        )

    prepared = prepare_editorial_source(
        EditorialSelectionRequest(scope=SourceScope(date_ranges=windows)),
        EditorialDependencies(source_fetcher=fetch),
    )

    assert seen_scopes[0].date_ranges == windows
    assert prepared.candidate_ids == ("first", "last")
    assert prepared.trace.story_of("gap").reason == "outside date scope"
...
def test_every_canonical_moment_is_contained_by_one_canonical_episode() -> None:
    """A midpoint cannot bridge independently grouped place threads."""
    noon = datetime(2026, 8, 25, 12, 0, tzinfo=UTC)
    first_open = make_asset("first-open", file_created_at=noon)
    second_place = make_asset("second-place", file_created_at=noon + timedelta(minutes=5))
    first_close = make_asset("first-close", file_created_at=noon + timedelta(minutes=9))
    midpoint = make_asset("midpoint", file_created_at=noon + timedelta(minutes=18))
    for asset, location in (
        (first_open, (50.0, 4.000)),
        (second_place, (50.0, 4.040)),
        (first_close, (50.0, 4.000)),
        (midpoint, (50.0, 4.020)),
    ):
        assert asset.exif_info is not None
        asset.exif_info.latitude, asset.exif_info.longitude = location
    ...
    assert tuple(group.candidate_ids for group in prepared.episode_groups) == (
        ("first-open", "first-close"),
        ("second-place", "midpoint"),
    )
    assert tuple(group.candidate_ids for group in prepared.moment_groups) == (
        ("first-open", "first-close"),
        ("second-place",),
    ...
    assert all(
        sum(
            set(moment.candidate_ids).issubset(episode.candidate_ids)
            for episode in prepared.episode_groups
        )
        == 1
...
def test_place_scope_keeps_only_assets_provably_at_the_place() -> None:
    """A place-scoped ask admits exif-proven presence; unknown place is not presence."""
    at_home = make_asset("at-home")
    elsewhere = make_asset("elsewhere")
    unplaced = make_asset("unplaced")
    assert at_home.exif_info is not None and elsewhere.exif_info is not None
    at_home.exif_info.city = "Brussels"
    elsewhere.exif_info.city = "Paris"

    prepared = prepare_editorial_source(
        EditorialSelectionRequest(scope=SourceScope(place=PlaceConstraint(cities=("brussels",)))),
        EditorialDependencies(source_fetcher=lambda _scope: (at_home, elsewhere, unplaced)),
    )

    assert prepared.candidate_ids == ("at-home",)
    assert set(prepared.excluded_ids) == {"elsewhere", "unplaced"}
    assert prepared.trace.story_of("elsewhere").reason == "outside place scope"
    assert prepared.trace.story_of("unplaced").reason == "outside place scope"


def test_place_scope_radius_admits_gps_within_and_drops_beyond() -> None:
    """The radius form reads raw GPS, so city-less frames at the place still enter."""
    near = make_asset("near")
    far = make_asset("far")
    assert near.exif_info is not None and far.exif_info is not None
    near.exif_info.latitude, near.exif_info.longitude = 50.880, 4.328
    far.exif_info.latitude, far.exif_info.longitude = 50.437, 5.971

    prepared = prepare_editorial_source(
        EditorialSelectionRequest(
            scope=SourceScope(place=PlaceConstraint(center=(50.878, 4.326), radius_km=1.0))
        ),
        EditorialDependencies(source_fetcher=lambda _scope: (near, far)),
    )
