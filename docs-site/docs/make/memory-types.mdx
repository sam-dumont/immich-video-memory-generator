...
worth reading about. Each type has its own normal target below. A thin period can produce a shorter
film. `--duration` on the CLI overrides the target, and so does **Auto duration** off on the
[Memory page](./web-ui.mdx).

| Type | Covers | Normal target |
|---|---|---|
...
Calendar memories keep their normal targets: one minute for a month and ten minutes for a year.
More photos give the editor more choices without increasing the target. Discovery can shorten a
film when there is not enough usable material. The run log and record say what set its length:

| What set it | When |
|---|---|
| `--duration` | you passed a number |
...
A trip and an album count active days: 30 s plus 10 s an active day, held between 60 s and 5 min. An
occasion counts active hours: 30 s plus 6 s an active hour, held between 60 s and 3 min. Other types
use their preset target. Twelve one-minute monthly memories do not make a twelve-minute yearly
memory: the year has its own ten-minute target.

Whatever the curve asks for, a film never runs longer than the varied excerpts its period can fill,
counted a day at a time and capped at 30 s a day. That is the same bound that stops a trip from
promising five minutes it cannot deliver.
...
the filter, not a different editor ([the curator](../being-rewritten/pipeline.md) edits all of them the same way).
...
The start comes from the birth dates Immich holds for the people you named, and from the
`birth_date` field in `people.yaml` where Immich has none. An AND waits for the youngest of the
people it needs; an OR opens with the oldest of the people it accepts, and claims nothing at all
when one of them has no birth date on record. A name nobody has a birth date for bounds nothing.

So two grandparents AND a child born in 2024 prepares 2024 onward instead of the whole library,
which is not a filter: no picture older than the child can hold the child. `runs why` on a picture
from before that day says the window was never typed and where it came from.

Dates you do give are never moved. If nobody named has a birth date anywhere, the ask is refused
the way it always was, and the message says why.

A person spotlight over several years is told as the person, not as a year: with no model, the
title screen is the person's name alone, and the film is read in the years that hold a picture of
them, so the empty years since a birth date take no part. On a model install it reads one account
per year, never one per month.

Every one of those years gets a shot before any story gets a second one, from the year's heaviest
story, as long as the year holds a picture that clears the usual bar. Without that, grants followed
the weight of each story alone, and the dense recent years took the film: a real 20-year person film
gave 44 of its 146 shots to one year and none to 2007, 2012 or 2014, which all hold pictures of her.
The same holds for a custom memory over several date ranges, which already promised every year of
its span a voice. A year whose only pictures fail the bar (a single scanned photo with nothing the
rules can read in it, say) still gets nothing, rather than a slot it cannot fill.
...
The name is the smallest place that holds at least 85% of the trip's located pictures, counted per
picture. It walks up from the city, to an island, to the region, to two regions, to the country,
and reads the city, region and country Immich stored on every picture from its own offline
GeoNames data. No outside call.

| The trip | Its name |
|---|---|
| A long weekend, nearly every picture in Las Vegas | `Las Vegas, United States` |
| A hike through five towns of Saxon Switzerland | `Saxony, Germany` |
| Two weeks across Heraklion, Rethymno and Chania | `Crete, Greece` (French film: `Crète, Grèce`) |
| A week in Paphos, Limassol and Larnaca | `Cyprus` (`Chypre`) |
| Bari, Polignano, Ostuni and Lecce | `Apulia, Italy` (`Pouilles, Italie`) |
| A road trip, mostly Utah and Nevada, a day in Arizona | `Utah and Nevada, United States` |
| Four states, none of them most of it | `United States` |

An island wins over the administrative units it is filed under: Palma and Pollença make
`Mallorca, Spain`, not `Balearic Islands`. The app carries a short offline list of islands for
that (the Greek, Balearic and Canary islands, Madeira, Bali, the main Hawaiian islands and a few
more); islands that are their own region or country (Crete, Sicily, Sardinia, Corsica, Cyprus) are
named that way already. Administrative wording is dropped (`Limassol District` becomes `Limassol`),
local spellings become the common name (`Puglia` becomes `Apulia`), and a region label in a script
the title language does not use names the trip one scale up instead.

Set `network.geocoding: true` and a trip that fits one city or one region is named by Nominatim
instead, in the film's language: the town when the pictures sit within 25 km of each other, else
the island or the state. Never the county, which in some countries is a regional unit with a long
local-script name. Islands, two-region trips and countries always come from the pictures. The
nightly `auto run` trip detector names trips the same way.
...
It asks in text and never sends pictures. The day arrives as one line per sampled picture, carrying
its capture time and whatever the library records about it: place, coordinates, recognised names,
whether it is a favourite, whether it is a video, and its caption when preparation has written one.
A day with nothing written about it at all is recorded as unjudged rather than guessed at.
...
Scope is that window when there is one, otherwise the day's **run**: the stretch of photographs the
occasion left, which the scan groups by five hours of silence rather than by midnight. The row
records `run_start` and `run_end`, and a run can outlast the date it began on: one catalogued day is
a long occasion that ran most of two days, and scoped to the date it began on the film
stopped at midnight with a quarter of its 379 pictures on the other side.
Rows written before the run was recorded fall back to midnight-to-midnight, and
`discover-days --replace` is what repairs them.

The run does not pad either. It never reaches past its own first and last picture, so a neighbouring
day it merely shares a date with stays out; a run reaching into a trip's dates is cut back at the
scan, and a run longer than 48 hours is not treated as one occasion at all.

The window shrinks and never pads, so a track day starts at the circuit rather than at the cat on
the balcony that morning. What decides whether a window is kept is how much of the run it holds, not
how long it runs: the row records `window_photos` beside `photos`, and a window holding fewer than
half of them is dropped in favour of the run itself. That track day spends 2.3 hours of a 10.6-hour
day in one place and still holds 92 % of its pictures, so it keeps its window. The occasion above,
whose recorded window held a few dozen of 379, does not: cut from that window the film saw a sliver
of the night and was refused for want of material. Rows written before the count existed are taken
as written.

Whichever of the three was used, and why, reaches the run record as `window_origin`, so
`immich-memories runs` can say a film was cut from the run rather than from the date.
...
1. `--title`, when you gave one.
2. The catalogue's title, when the day has a row with one.
3. The model, writing an occasion title from the day's own facts (its places, its people, the album
   the day sits in) wherever a reader is configured. It is given no catalogue entry and invents no
   event name.
4. The day's date, when there is no reader and nothing else named it.
...
A catalogued day is still the better film, because the scan wrote its title against the day's
evidence months earlier and can bound the occasion inside it. An uncatalogued day gets the whole
day and a title written at render time.
