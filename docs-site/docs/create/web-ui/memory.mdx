...
The pool is everything the brief found: videos, Live Photos and, with the switch on, photos, grouped by month with an **Include** checkbox each. Everything starts checked. Checked media is what the editor cuts from, so untick what may never be used: the accidental pocket recording, the screenshot you forwarded. Twins and bursts are the editor's to judge; nothing here pre-deselects them.

The page has its own **Generate Memories** button, which cuts from the checked pool with the same duration line as the brief, and after a cut a **Review & Refine** screen: one expansion per selected clip with an **Include in compilation** checkbox, a **Select range** slider for the excerpt (plus **First 5s**, **Middle 5s**, **Last 5s**, **Full clip**, **Preview**) and a **Rotation** select. Your edits are projected back onto the cut; the final duration updates as you trim.
...
While it runs the page shows one row per phase (**Finding media**, **Loading thumbnails**, **Reading the pictures**, **Editing**, **Done**) with the run's own stage beside the active row, the elapsed time, and **Cancel**, which stops after the current stage. The stages a run reports, in order: *Preparing source metadata* (annotation preparation; on a library that has never been read this is the slow one), then *Reading event evidence*, *Reading the period account*, *Building editorial cards*, *Editing the memory* and *Validating selected source timing*. What each does is in [The Curator](../pipeline/the-curator.md).

The preparation pass counts its work, so while it runs you also get a progress bar with its own count and a strip of the last twelve pictures it read: on a first cut over a large library that pass reports tens of thousands of items and is the only evidence anything is moving. **Details** folds open the run's stage lines, newest last, the last few dozen of them; it stays closed, so the rows are what you see unless you ask.
