...
their own hardware ([Running modes](../being-rewritten/running-modes.md)). An NVIDIA card can run both the encoder
and the detectors, but the detectors go through CUDA in the
[inference service](../better/inference.md), not through NVENC.
...
[inference service](../better/inference.md) and preparation changes shape: on the
...
[Linux + NVIDIA](../being-rewritten/linux-nvidia.md) has a full compose file.
...
of the [inference service](../better/inference.md).
...
and the caption server up is [steps 3 and 4 of the self-hosting guide](../being-rewritten/self-hosting.md#3-serve-the-reader).
...
[one machine or two](../being-rewritten/self-hosting.md#one-machine-or-two) before sizing anything. The caption server
...
[Running modes](../being-rewritten/running-modes.md#what-to-expect-on-a-first-run). The CPU-only row to read is the
...
[inference service](../better/inference.md) on a box that has one.
