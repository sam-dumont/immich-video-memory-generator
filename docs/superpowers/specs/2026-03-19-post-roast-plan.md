...
### Phase 2 — Video Quality (#72) ✅ DONE (PR #79)
...
**Deliverables:**
- [x] Single-encode assembly: `assemble_scalable` builds one FFmpeg filter graph from source clips
- [x] Removed pre-encode step + 270 lines of dead code (xfade chain, concat helpers)
- [x] CRF 18 confirmed correct for single-pass (was effectively CRF ~22 before with dual encoding)
- [x] Verified downscaler files don't leak (glob pattern `{id}.*` excludes `{id}_480p.*` by design)
- [x] HDR/SDR detection: cross-checks transfer function AND color primaries (bt2020 required)
- [x] zscale color accuracy: `npl=203` + `agamma=false` fixes red cast on SDR→HLG conversion
- [x] HDR gain map extraction: Apple HEIC (pillow-heif) + UltraHDR JPEG (Android/Pixel/Samsung)
- [x] Dynamic `peak_nits` from gain map metadata (not hardcoded)
- [x] Chunked assembly resolution consistency (all batches use same target resolution)
- [x] Temp dir cleanup (.intermediates, .live_merges, .assembly_temps)
- [x] Test fixtures: Jellyfin HDR10 + DV P8.4, 6 UltraHDR gain map JPEGs (MIT + CC BY-SA)
- [ ] Integration test: SSIM/VMAF quality threshold (follow-up)

**Files:** `processing/assembly_engine.py`, `processing/ffmpeg_filter_graph.py`, `processing/hdr_utilities.py`, `processing/filter_builder.py`, `processing/video_assembler.py`, `photos/animator.py`, `photos/photo_pipeline.py`, `generate.py`
