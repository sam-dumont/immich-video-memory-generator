...
from immich_memories.analysis.period_insight_material import (
    EpisodeScanPack,
    EpisodeSheet,
    _episode_material,
    _solo_episode_pack,
...
class PassZeroResult:
    ...
    warnings: tuple[str, ...]
    actual_calls: int
    # Packs asked in recovery (a solo episode page, a lone-episode retry). They
    # are not the packing plan, but their answers carry readings and cull lists.
    fallback_packs: tuple[EpisodeScanPack, ...] = ()
...
def run_period_insight(
    ...
    *,
    ...
    observations, banked, scan_attempts, fallback_packs = _read_episode_packs(
        episode_packs,
        requester=requester,
        limits=request_limits,
...
def _read_episode_packs(
    ...
    *,
    ...
    tuple[EpisodeScanPack, ...],
]:
    if not 1 <= concurrency <= 8:
        raise ValueError("episode scan concurrency must be between 1 and 8")
    if concurrency == 1:
        outcomes = tuple(
            _read_episode_pack(pack, requester, limits, atlas=atlas, output_dir=output_dir)
            for pack in packs
        )
    else:
    ...
    return (
        tuple(
            observation
            for observations, _scans, _attempts, _packs in outcomes
            for observation in observations
        ),
        tuple(scan for _o, scans, _a, _p in outcomes for scan in scans),
        tuple(attempt for _o, _s, attempts, _p in outcomes for attempt in attempts),
        tuple(pack for _o, _s, _a, packs in outcomes for pack in packs),
    )
...
def _read_episode_pack(
    ...
    *,
    ...
    """Read one independent pack; what it cannot answer whole is re-asked per episode.

    A shared page carries several complete episodes. An episode the packed
    answer left unread (a timeout, a runaway that hit the output cap, an
    invalid entry) is rendered on a page of its own and asked about alone: a
    fraction of the answer for the same pixels. A lone episode whose transport
    failed gets one identical retry; one that answered invalidly does not,
    since temperature 0 would repeat it. Measured 2026-09-01 on a real year:
    4 of 77 packs timed out and 1 ran away, and each lost every episode on its
    page for the whole wall. Re-read packs are returned so the cull reads them.
    """
    observations, scan, attempt = _ask_episode_pack(pack, requester, limits)
    scans: tuple[BankedEpisodeScan, ...] = () if scan is None else (scan,)
    attempts: tuple[EpisodeScanAttempt, ...] = (attempt,)
    missing = tuple(
        scope
        for scope, observation in zip(pack.scopes, observations, strict=True)
        if scope.tile_refs and observation.reading is None
    )
    if not missing:
        return observations, scans, attempts, ()
    if len(pack.scopes) == 1:
        if attempt.answer is not None:
            return observations, scans, attempts, ()
        retry = replace(pack, pack_id=f"{pack.pack_id}-retry")
        observations, scan, attempt = _ask_episode_pack(retry, requester, limits)
        scans += () if scan is None else (scan,)
        return observations, scans, (*attempts, attempt), (retry,)
    recovered: dict[tuple[str, str], EpisodePageObservation] = {}
    fallback_packs: list[EpisodeScanPack] = []
    for scope in missing:
        solo = _solo_episode_pack(scope, atlas=atlas, output_dir=output_dir)
        solo_observations, solo_scan, solo_attempt = _ask_episode_pack(solo, requester, limits)
        recovered[(scope.episode_id, scope.page_id)] = solo_observations[0]
        scans += () if solo_scan is None else (solo_scan,)
        attempts += (solo_attempt,)
        fallback_packs.append(solo)
    observations = tuple(
        recovered.get((observation.episode_id, observation.page_id), observation)
        for observation in observations
    )
    return observations, scans, attempts, tuple(fallback_packs)


def _ask_episode_pack(
    pack: EpisodeScanPack,
    requester: EditorialGateway,
    limits: VisionRequestLimits,
) -> tuple[
    ...
    """Ask the model about one pack exactly once, fail-open."""
