...
This page is being split into the new docs. Its text moves to [On a NAS](../run/nas.md), [Render on a GPU box](../better/gpu-render.md).
...
this layout ([Caption server](../better/captions.md#apple-silicon-with-mlxcel)).
...
the LAN instead, turn on [authentication](../run/authentication) first (the app holds an
...
[inference service](../better/inference.md) took this NAS from 1.4813 s a picture to
...
the rest cost EUR 0.054 to EUR 0.585 of tokens: [Readers](../better/reader.md). Every outbound request is
listed on [Network & Privacy](../run/privacy.md).
...
[Caption server](../better/captions.md) has the flags.
...
[Intel Quick Sync](../run/hardware.md#intel-quick-sync-and-amd-vaapi) has the compose block. ARM models
...
- Title screens on the GPU kernels. The J4125 has no AVX, so the kernel renderer is out entirely
  and every title is PIL-rendered: [CPUs without AVX](../run/hardware.md#cpus-without-avx).
...
[Daily automation](../run/docker.md#daily-automation).
