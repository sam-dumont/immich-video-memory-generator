...
On the CLI there is no such planner. `--duration` left out means no target at all, and the editor
decides the length from what it kept. The 60 to 300 second range some of these pages quote belongs
to the trip planner, which albums do not use.
...
The 9 KB is the measured figure; multiply it by your album size for the memory. The read times
that used to be quoted here had no provenance, so they are gone.
