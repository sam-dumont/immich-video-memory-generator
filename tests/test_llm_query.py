...
class TestQueryLlmOpenAI:
    ...
    async def test_a_200_provider_error_is_reported_before_choices_are_read(self):
        ...
        # WHY: httpx post is the transport boundary to the provider; error mapping stays real.
        with (
...
async def test_transport_observer_records_connection_errors() -> None:
    ...
    attempts = []
    # Transient drops retry with backoff (owner ruling 2026-09-01) before the
    # final raise; each drop is observed.
    # WHY: no response object exists when the wire connection itself fails.
    with (
        # WHY: the provider is the external boundary; every attempt must see a dropped connection
        patch("httpx.AsyncClient.post", side_effect=httpx.ConnectError("offline")),
        # WHY: backoff sleeps are wall-clock; the test asserts the retry count, not the delay
    ...
    assert [(event.attempt, event.outcome, event.status_code) for event in attempts] == [
        ...
        (2, "connection_error", None),
        (3, "connection_error", None),
    ]
...
@pytest.mark.asyncio
async def test_a_transient_transport_drop_is_retried_not_fatal():
    """Owner ruling 2026-09-01: a peer-closed connection killed a 26-minute run.
    The transient transport family retries with backoff before anything raises."""
    import httpx
