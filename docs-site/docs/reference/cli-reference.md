...
```bash
immich-memories cache [OPTIONS] COMMAND [ARGS]...
```
...
| `--memory-type` | `year_in_review`, `season`, `person_spotlight`, `multi_person`, `monthly_highlights`, `on_this_day`, `trip` | - | Memory type preset |
| `--season` | `spring`, `summer`, `fall`, `autumn`, `winter` | - | Season (use with --memory-type season) |
| `--month` | integer | - | Month 1-12 (narrows any yearly memory type; selects trip by month) |
| `--hemisphere` | `north`, `south` | north | Hemisphere for season calculation |
| `--duration`, `-d` | integer | - | Target duration in seconds (default: from memory type preset) |
| `--orientation`, `-o` | `landscape`, `portrait`, `square` | landscape | Output orientation |
| `--scale-mode`, `-s` | `fit`, `fill`, `smart_crop`, `blur` | - | Scale mode (default: from config or smart_crop) |
| `--transition`, `-t` | `smart`, `cut`, `crossfade`, `none` | smart | Transition style (default: smart — mix of fades & cuts) |
| `--resolution`, `-r` | `auto`, `4k`, `1080p`, `720p` | - | Output resolution (default: config value, 'auto' to match source clips) |
| `--music-volume` | float | 0.5 | Music volume 0.0-1.0 (default: 0.5) |
| `--format` | `mp4`, `h265`, `prores` | - | Output format override (default: config value) |
| `--quality`, `-q` | `high`, `medium`, `low` | - | Output quality (default: from config, typically high) |
...
| `--analysis-depth` | `fast`, `thorough` | - | Analysis depth: fast (metadata gap-fill) or thorough (LLM gap-fill) |
...
| `--tempo`, `-t` | `slow`, `medium`, `fast` | - | Tempo |
...
| `--status`, `-s` | `completed`, `failed`, `running`, `cancelled`, `interrupted` | - | Filter by status |
...
```bash
immich-memories scheduler [OPTIONS] COMMAND [ARGS]...
```
...
| `--orientation`, `-o` | `landscape`, `portrait`, `square` | landscape | Output orientation |
| `--resolution`, `-r` | `720p`, `1080p`, `4k` | 1080p | Output resolution |
| `--locale`, `-l` | `en`, `fr` | en | Language |
| `--style`, `-s` | `modern_warm`, `elegant_minimal`, `vintage_charm`, `playful_bright`, `soft_romantic`, `random` | random | Visual style |
| `--output`, `-O` | path | - | Output file path |
| `--type` | `title`, `month`, `ending` | title | Screen type |
