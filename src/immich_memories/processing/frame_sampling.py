...
def extract_frame_at(
    ...
    *,
    ...
    """Decode exactly one requested timestamp through the shared FFmpeg path."""
    if timestamp < 0 or width <= 0 or not _usable_video(video):
        return None
    output_path.parent.mkdir(parents=True, exist_ok=True)
    return output_path if _extract_one(video, timestamp, width, output_path) else None
