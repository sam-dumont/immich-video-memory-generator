...
worth reading about. The length below is what a type asks for at ordinary density, not a cap and not
a promise: the run fits it to the material the period turns out to hold. `--duration` on the CLI
overrides that, and so does **Auto duration** off on the [Memory page](./web-ui.mdx).

| Type | Covers | Length at ordinary density |
|---|---|---|
...
A film's length is decided after discovery, not before it, so a thin month gets a short film and a
crowded one gets a longer film. Four things can set it, and the run log and the run record both say
which one did:
...
occasion counts active hours: 30 s plus 6 s an active hour, held between 60 s and 3 min. Every other
type counts **photographed days** against the days its period covers. One day in three carrying
pictures is the density the lengths above were written for, so a period at that density keeps its
length; a thinner period shortens and a denser one grows, between half and one and a half times it.
Growth follows the square root of the density, because the twentieth photographed day adds less to a
film than the second.
...
a birth that ran 42.5 continuous hours across two dates, and scoped to the date it began on the film
...
day in one place and still holds 92 % of its pictures, so it keeps its window. The birth above,
