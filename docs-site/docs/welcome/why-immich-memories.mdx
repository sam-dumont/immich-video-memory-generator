...
A tool that does the boring part for me. It connects to Immich (read-only by default; `--upload-to-immich` is the one thing that writes back), looks at a time period I pick, and figures out which clips are worth keeping. It scores them on motion, faces, audio content, and optionally runs an LLM to understand what's actually happening in the scene. Then it assembles everything into a video with title screens, transitions, and music.
