...
def test_episode_packs_run_concurrently_without_reordering(monkeypatch) -> None:
    ...
    def read_one(pack, _requester, _limits, **_kwargs):
        ...
        with lock:
            active -= 1
        return ((pack.pack_id,), (), (pack.pack_id,), ())
    ...
    observations, banked, attempts, fallback_packs = period_insight._read_episode_packs(
        packs,
        requester=object(),
...
def _assert_next_episode_would_overflow(packs, *, max_output_tokens: int = 4000) -> None:
    ...
    from immich_memories.analysis.period_insight_material import MAX_EPISODES_PER_PACK
...
def test_fused_episode_v4_request_cannot_reuse_a_legacy_v3_bank(
    ...
    ) == ("episode-scan-v4", "episode-scan-prompt-v5", "episode-scan-v4")
    legacy = replace(
        current,
...
def test_pass_zero_retains_the_atlas_and_failed_pack_provenance(tmp_path: Path) -> None:
    """A timeout keeps reusable pixels, the failed request association, and earns one retry."""
    from immich_memories.analysis.editorial_gateway import VisualEditorialGateway
    from immich_memories.analysis.llm_query import LLMTransportAttempt
    from immich_memories.analysis.period_insight import run_period_insight
    ...
    # a lone episode gets one identical retry under its own pack identity
    assert [attempt.answer for attempt in result.scan_attempts] == [None, None]
    ...
    assert result.scan_attempts[1].pack_id == f"{result.episode_packs[0].pack_id}-retry"
    assert result.scan_attempts[1].request_trace is prepared.trace.requests[1]
    assert [pack.pack_id for pack in result.episode_packs] == [result.scan_attempts[0].pack_id]
    assert [pack.pack_id for pack in result.fallback_packs] == [result.scan_attempts[1].pack_id]
...
def test_one_packed_raw_answer_banks_valid_siblings_when_one_episode_is_invalid(
    ...
    The packed envelope banks whole even though one entry is invalid; that
    entry's episode is then re-asked alone on its own page and read.
    """
    from immich_memories.analysis.editorial_gateway import VisualEditorialGateway
    from immich_memories.analysis.llm_query import LLMTransportAttempt
    from immich_memories.analysis.period_insight import run_period_insight
    ...
    async def _answer(prompt, _config, **kwargs):
        ...
        assert len(scopes) in (1, 3)
        entries = [
            {
                "episode": int(episode_alias),
                "page": int(page_alias),
                "visual_summary": f"Visible episode {episode_alias}",
                # packed: each episode names its own tile, the middle one an
                # impossible tile; alone: the only tile on the page
                "representative_tiles": [
                    (99 if index == 1 else index + 1) if len(scopes) == 3 else 1
                ],
    ...
    # the packed envelope plus the solo re-read of the invalid sibling
    assert len(result.banked_scans) == 2
    assert [item.reading is not None for item in result.page_observations] == [True, True, True]
    assert tuple(item.episode_id for item in result.episode_readings) == tuple(
        group.group_id for group in prepared.episode_groups
    )
    assert [len(pack.scopes) for pack in result.fallback_packs] == [1]
    # pack, solo re-read, then the wall
...
def test_banked_episode_scan_can_be_reparsed_after_a_later_physical_failure(
    ...
    # two continuations, the failed second one retried, then the period wall
    assert calls == 4
    assert len(result.banked_scans) == 2
    ...
    # two continuations, one retry, one wall: the reparse adds nothing to them
    assert before == after == (4, 4, banked.answer.provenance.request_key)
...
def test_an_unread_episode_costs_its_own_pictures_not_the_month(tmp_path: Path) -> None:
    ...
    async def _answer(prompt, _config, **kwargs):
        ...
        kwargs["transport_observer"](LLMTransportAttempt(1, "response", 200))
        if "chronological episode pack" in prompt:
            # the first pack refuses, and so does each of its episodes asked
            # alone; the other packs answer
            if calls == 1 or len(re.findall(r"episode=(\d+) page=", prompt)) == 1:
...
def test_a_failed_shared_pack_is_re_read_one_episode_at_a_time(tmp_path: Path) -> None:
    """A pack the model cannot answer whole costs no episode its reading.

    Three complete episodes share one page. The packed request fails; each
    episode is then asked about alone on the same page, and all three read.
    """
    from immich_memories.analysis.editorial_gateway import VisualEditorialGateway
    from immich_memories.analysis.llm_query import LLMTransportAttempt
    from immich_memories.analysis.period_insight import run_period_insight
    ...
    # WHY: query_llm is the provider boundary; pack layout, fallback and parsing stay real.
    with patch("immich_memories.analysis.editorial_gateway.query_llm", new=_answer):
        result = run_period_insight(
            prepared,
            requester=gateway,
    ...
    assert scope_counts[:4] == [3, 1, 1, 1]
    # the packing plan is untouched; each re-read episode is a solo pack
    assert len(result.episode_packs) == 1
    assert [len(pack.scopes) for pack in result.fallback_packs] == [1, 1, 1]
    assert all(item.reading is not None for item in result.page_observations)
    assert len(result.episode_readings) == 3
    assert len(result.banked_scans) == 3
    assert [attempt.answer is None for attempt in result.scan_attempts] == [
        True,
        False,
        False,
        False,
