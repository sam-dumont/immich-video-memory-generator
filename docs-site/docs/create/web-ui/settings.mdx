...
Three pages sit outside the memory flow, reachable from the sidebar at any time: **Config** (`/settings/config`), **People** (`/settings/people`) and **Cache** (`/settings/cache`). People is documented with [`immich-memories people`](../cli/people.md), which writes the same file.
