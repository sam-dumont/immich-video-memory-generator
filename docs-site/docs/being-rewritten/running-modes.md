...
This page is being split into the new docs. Its text moves to [Requirements and tiers](../run/requirements.md), [What a model adds, what it costs](../better/overview.md), [Measured](../better/measured.md), [Render on a GPU box](../better/gpu-render.md).
...
optional [inference service](../better/inference.md) on a CPU or CUDA box, switched
...
thousand pictures is about four days: [NAS + a model box](./nas-only.md).
...
What each model did is on [Readers](../better/reader.md).
...
[the hardware overview](../run/hardware.md#what-the-card-is-actually-worth), from an earlier run. This
...
[Network & Privacy](../run/privacy.md).
...
| Cluster, rules reader | a Kubernetes Job plus the [inference service](../better/inference.md) on a card. `reader: rules`, `tier: no_captions` | 1 h 12 min | 8 min | dates, places, favourites, known people and classifier facts. No thesis, no reason. 17 % overlap with the reference cut | nothing |
...
[Readers](../better/reader.md) has what each model did, and what the one priced hosted cell cost.

A fourth layout, a NAS with the app and a model on another box, is documented and its render is
measured, but no real month has run on it: [NAS + a model box](./nas-only.md). The
...
comparison with a different cut. [NAS setup](./nas-only.md#let-the-gpu-box-render)
...
[Title kernels](../run/hardware.md#title-kernels).
