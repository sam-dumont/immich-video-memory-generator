...
- **Auto duration stays watchable**: Auto starts at 30 seconds plus 10 seconds per active day, bounded to 60–300 seconds when there is enough material. Seven dense days produce about 1:40; 12 dense days produce 2:30. It then checks the selected media's usable excerpts and shortens sparse trips instead of promising time it cannot fill. Turn off the **Auto duration** switch in Step 2 (or pass `--duration` on the CLI) when you want an exact runtime.
