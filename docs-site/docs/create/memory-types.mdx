...
`--birthday` turns a person spotlight into a different memory: the year **ending** on that year's
birthday, plus each of the five previous birthdays as a ±1 day cutaway, reading the birth date from
Immich. See [Birthday compilations](./recipes.md#birthday-compilations).
...
Nothing searches for it. [`discover-days`](./cli/discover-days.md) walks the library a month at a
