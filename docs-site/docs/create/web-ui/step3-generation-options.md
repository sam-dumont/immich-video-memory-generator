...
Choose the transition style between clips. The default `smart` mode picks the best transition per cut point (crossfade for slow scenes, hard cut for fast action).

## Music

Three options:

- **AI-generated**: Uses your configured backend (ACE-Step, MusicGen, or both) to generate a soundtrack that matches the video's mood. See [Audio & Music](../pipeline/audio-and-music.md).
