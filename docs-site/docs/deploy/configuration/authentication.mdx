...
(The Docker image passes `--host 0.0.0.0` explicitly, which inside a container is the only
...
:::warning The UI reads auth from the default config path, always
`immich-memories --config PATH ui` uses `PATH` to pick the host and port, and then the server reads
`~/.immich-memories/config.yaml` for every authentication decision. So an `auth:` block that lives
only in `PATH` is ignored, and the two files can disagree about whether auth is on: the bind
address comes from one and enforcement from the other. Keep your auth config in the default path,
or in environment variables, which apply either way.
:::
...
With Docker there is nothing to install: both published images are built with all extras, authlib included.
...
**How it works:** The middleware reads the `Remote-User` header (configurable) and creates a session automatically. If the peer is not in `trusted_proxies`, the headers are not read at all and the request is simply unauthenticated. Nothing is stripped, so do not rely on that: an upstream that forwards the header from an untrusted source is your problem to fix, not the app's.
...
2. **List the proxy in `auth.trusted_proxies`.** The app passes it to uvicorn as
   ...
   peer for uvicorn's purposes; only use it when the app is unreachable except through the proxy.
   It does **not** work with `provider: header`: the app parses each entry as an IP or CIDR and
   silently drops what it cannot parse, so `["*"]` passes validation and then trusts nobody,
