...
def _validate_rendering_family_ids(family: LivePhotoRenderingFamily) -> None:
    if family.material is not None:
        family.material.assert_arrays(
            still_ids=family.still_ids,
            video_ids=family.video_ids,
            trim_points=family.trim_points,
            shutter_timestamps=family.shutter_timestamps,
        )
        return
...
def _validate_rendering_family_timing(family: LivePhotoRenderingFamily) -> None:
    ...
    if family.material is not None:
        return  # Canonical material already validates source and segment chronology.
    ...
    *,
    ...
    if material is not None:
        material.assert_arrays(
            ...
            shutter_timestamps=shutter_timestamps,
        )
        version = "live-photo-rendering-family-v2-positive-segments"
        ...
        return f"{version}-{sha256(material_key.encode()).hexdigest()}"
    manifest = {
