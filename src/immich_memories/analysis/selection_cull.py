...
def run_cull(
    ...
    *,
    ...
    readings = tuple(
        ...
        for pack in (*pass_zero.episode_packs, *pass_zero.fallback_packs)
    )
    rejects, pass_one_warnings, logical_requests = _combine_pack_readings(readings)
    result = run_cull_decisions(
        prepared,
        rejects,
        provenance=_pass_provenance(prepared, pass_zero, logical_requests),
        warnings=pass_one_warnings,
        request_traces=logical_requests,
        verdicts=verdicts,
    )
