...
| Who may see it? | Any flag from the detectors keeps a picture at family-only viewing, and the distilled `uncovered_person` head is a second opinion that can add one. A picture in a five-minute capture run that is at least half flagged, with at least three flagged captures in it, is held with the run (`exposure_chain`). A `no` from any of it lifts nothing: nothing clears a flag except you, on the pool page |
