...
[caption server](./caption-server.md). Two services, one default port: on one host, move one.
...
each measured is on [Running modes](../running-modes.md#what-dominates-per-host).
