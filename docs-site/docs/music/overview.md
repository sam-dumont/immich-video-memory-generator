...
2. **Music generation** — The multi-provider pipeline takes that mood and sends it to a music generation API. It tries backends in priority order: ACE-Step first, falls back to MusicGen if ACE-Step is unavailable or fails.
