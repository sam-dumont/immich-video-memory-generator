...
:::warning Secure cookies over HTTPS

If you run behind a reverse proxy with HTTPS (Traefik, Caddy, nginx -- most setups do), make sure the proxy sets `X-Forwarded-Proto: https` so the app knows the connection is encrypted. Without this, the session cookie may be sent over plain HTTP on internal hops, which means an attacker on the same network could intercept it and hijack the session.

Most reverse proxies set this header by default. If yours doesn't, add it explicitly:
...
```yaml
# Traefik — enabled by default when using entrypoints with TLS
```

When running without a reverse proxy (e.g. direct access on localhost for development), this is not a concern.
:::
