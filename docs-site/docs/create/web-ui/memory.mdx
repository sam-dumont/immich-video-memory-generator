...
`/` is one page with three states: the **brief**, the **cut** in progress, and the **story** the cut produced. It reads like `immich-memories generate`: one memory type, that type's parameters, one duration, one button. The sidebar calls it **Memory**. **Media pool**, **Options** and **Export** let you review media
...
A **Grouped people condition** says what the toggle cannot: quoted names with AND, OR and parentheses, `--people-expression` on the CLI. It lives under **Advanced people condition**, folded away under the picker, The panel opens when a condition is already set. Typing a condition replaces what the picker and the toggle say; touching either of them clears the condition.
...
Three tabs: **Year** (a year, then **Calendar Year** or **From Birthday**, a birthday-to-birthday range), **Duration** (1–24 months or years from a **Starting from** date), **Custom Range** (start and end dates). The resulting range and its length in days show under the tabs; with Auto duration on, the target follows the range at roughly 10 minutes per year, bounded to 1–60 minutes.
...
**Advanced**, at the bottom of the brief, holds:
...
While it runs the page shows one row per phase (**Finding media**, **Loading thumbnails**, **Reading the pictures**, **Editing**, **Done**) with the run's own stage beside the active row, the elapsed time, and **Cancel**, which stops after the current stage. The stages a run reports, in order: *Reading dates, places and people* (annotation preparation; on a library that has never been read this is the slow one), then *Reading event evidence: 3/12* (one count per request to the reader), *Reading the period account: page 2*, *Building editorial cards*, *Editing the memory: 22 pictures into the audience gate* (then the count into the picture review and after the duplicate review) and *Validating selected source timing*. What each does is in [Pipeline Overview](../pipeline/pipeline-overview.md).

Counted stages show progress and an estimate. Preparation also shows the last twelve pictures
processed. **Details** expands the recent stage log. A dropped reader connection names the
endpoint and retries, with waits of two and four seconds before giving up. A server that accepts
a connection but stalls may wait until its configured timeout.

One session runs one cut at a time, shared across tabs. Reloading rejoins its progress; cancelled,
failed or interrupted attempts show **Cut again**. If the cut finished while the session lost
its media, **Re-load media to export** restores the pool and saved selection. Attempt records
live under the cache directory in `editorial-runs/`.
...
**Preview & Export** opens with stat cards (Clips, Photo Pool, Duration, Format) and the **Output** card: the **Output filename** (built from the type, the people and the range, as in `everyone_jan-apr_2025_memories.mp4`), the "Will be saved to" line (`output.directory`, `~/Videos/Memories` by default; the file lands in a per-run subfolder, `<name>_<YYYYMMDD_HHMMSS_xxxx>/`; use the final **Saved to** path), and **Upload after generation** with an **Album name** (off by default); uploading creates a video asset and can create the named album. A matching generated filename
in that album can supersede an earlier copy; check the delivery result.
