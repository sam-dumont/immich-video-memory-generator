...
def source_fingerprint(server_url: str, buckets: list[TimeBucket]) -> str:
    """Identify the server and cheap library snapshot without storing its URL."""
...
def load_or_fetch_trip_assets(
    ...
    *,
    ...
    fingerprint = source_fingerprint(server_url, buckets)
    cache = TripInputCache(cache_root)
    cached = cache.load(fingerprint, requested_range, now=now)
    if cached is not None:
        logger.info("Trip input cache hit: %d assets", len(cached))
        return cached

    from immich_memories.api.all_assets_service import AllAssetsService

    service = AllAssetsService(client._async_client.search)
    assets = client._run(service.get_assets_for_date_range(requested_range))
    logger.info("Fetched %d assets for trip detection", len(assets))
    cache.store(fingerprint, requested_range, assets, now=now)
    return assets
