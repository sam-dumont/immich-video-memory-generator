...
| `llm.base_url` | the reader | 800 px tiles of a few dozen candidates and their annotation lines, which carry people and place names; for titles, names, places, dates, descriptions, and the relationships your people file records between the people in the film | `reader: rules`, or a local model (default `localhost:8080`, the app's own port, so set it) |
