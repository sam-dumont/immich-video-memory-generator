...
def _mock_extract(clips_with_paths: list[tuple[Path, str, float]]):
    """Build a patch for extract_clips that returns AssemblyClips from local files."""
    ...
    return patch("immich_memories.generate_render.extract_clips", side_effect=_extract)
...
class TestGenerateFlowErrors:
    ...
    def test_all_clips_invalid_raises_error(self, tmp_path):
        ...
        with patch("immich_memories.generate_render.extract_clips", return_value=[]):
            params = GenerationParams(
                clips=[clip],
