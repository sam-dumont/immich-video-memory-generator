...
def has_camera_exif(asset: Any) -> bool:
    """Whether the file carries a camera make or model at all.

    The one provenance fact every source rule reads, so it is answered once
    here rather than re-derived by each of them.
    """
    exif = getattr(asset, "exif_info", None)
    return bool(exif and (getattr(exif, "make", None) or getattr(exif, "model", None)))


def source_dimensions(asset: Any, clip: Any = None) -> tuple[int, int]:
    """Probed motion dimensions when a clip has them, then Immich's asset facts."""
    if clip is not None and clip.width and clip.height:
        return clip.width, clip.height
    return getattr(asset, "width", 0), getattr(asset, "height", 0)


def forwarded_source_refusal(asset: Any, *, min_short_side: int, clip: Any = None) -> str | None:
    """Why this file's size and provenance read as a forwarded copy, or None.

    The half of the camera-roll provenance rule that looks at the pixels rather
    than the name, shared by the camera-roll filter and editorial admission so
    the two cannot drift. `clip` supplies probed motion dimensions where the
    pool has them.
    """
    if min_short_side <= 0 or getattr(asset, "is_favorite", False):
        return None
    width, height = source_dimensions(asset, clip)
    if is_usable_source(
        width=width,
        height=height,
        has_camera_exif=has_camera_exif(asset),
