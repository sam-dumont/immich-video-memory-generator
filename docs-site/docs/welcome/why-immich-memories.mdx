...
- **An editor, not a scorer**: so I don't have to watch 400 clips to find the 25 that tell the story
- **Face recognition**: so I can ask for a memory of one person and get one, and so a photo's Ken Burns move lands on a face rather than a wall
- **Sameness, judged**: so I don't get three versions of the same moment from a held shutter or a re-import
...
- **Audio ducking**: so music drops under the clip's own sound instead of talking over it
