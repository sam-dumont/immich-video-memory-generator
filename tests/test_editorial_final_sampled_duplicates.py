...
def test_episode_variants_are_compared_despite_different_framing_and_captions():
    ...
    def episode_relation(left, right, _distance=None):
...
def test_a_hash_close_episode_neighbour_keeps_the_measured_strict_question():
    original = [
        unit("first-take", event="ceremony"),
        unit("second-take", event="ceremony"),
    ]
    distances = []

    def strict(left, right, distance=None):
        # WHY: the direct visual comparison is the external model boundary.
        distances.append(distance)
        return {"same": True, "source_pair": [left, right]}

    kept, audit = reduce_final_sampled_duplicates(
        original,
        picture_records={key: {"status": "available"} for key in ("first-take", "second-take")},
        preview_hashes=dict.fromkeys(("first-take", "second-take"), "0000000000000000"),
        confirm_relation=strict,
        confirm_episode_relation=lambda *_: pytest.fail("pixels already corroborate this pair"),
    )
    assert [row["asset_id"] for row in kept] == ["first-take"]
    assert distances == [0] and audit["relation_checks"] == 1
