...
A Holiday memory combines the same occasion across several years. Each year contributes
a separate date window, so five Christmases do not include the months between them.
...
These are fixed calendar conventions; Mother's Day, Father's Day and Thanksgiving may
fall on different dates in your country.
...
Use it for a recurring celebration or household anniversary. A year without matching
material contributes nothing.

## In the web UI

Pick **Holiday** on the [Memory page](../web-ui/memory.mdx#memory-types-and-their-parameters). It takes the holiday, the most recent year to include, how many years to span, and an optional **Only with** people filter. The editor reads the available windows as one period.
