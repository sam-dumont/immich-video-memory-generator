...
**Trip** finds trips from GPS-tagged photos, videos and Live Photos, using the same discovery as the CLI. The year picker includes years with only photos. Discovery includes a month on either side of the selected year, so a trip across New Year keeps its full dates; only trips overlapping the selected year are offered. The dropdown counts assets, including photos.
...
Counted stages also show an estimate such as **~36s left in this stage**, measured from that
stage's completed work. It appears after the count advances beyond the first update. A new
stage starts with no estimate; previews and captions have different costs. The CLI shows the
same saved value, and reloading the page keeps it. Slow items can change the estimate.
...
- one line with the number of pictures, the seconds of pictures and video they add up to, and about how long the whole film runs; titles and transitions make up the difference. The film's length is an estimate until the file exists: smart transitions decide fade or cut boundary by boundary, so the overlap they take moves a second either way from one render to the next;
...
The label describes the last cut. Changing an **Include** checkbox changes the
next cut's input, so the label stays put until you cut again.

The compact grid carries the same label under each thumbnail:

<ThemedScreenshot name="memory-pool-outcomes-grid" alt="The compact grid: the same outcome line under every thumbnail" />

Before your first cut there is no outcome to show, and the pool renders none.
