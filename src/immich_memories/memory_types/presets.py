...
def person_filter_for(
    ...
    *,
    ...
    what ``--person Riley --person Bob`` has always fetched and what a
    ...
    if person_expression is not None:
        if names and set(names) != set(person_expression.leaf_values):
            raise ValueError("person names and grouped condition disagree")
        return PersonFilter(
            mode="expression",
            person_names=list(person_expression.leaf_values),
