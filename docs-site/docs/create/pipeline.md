...
A run is held to family viewing as a whole when it is at least **three** captures long and at least
**half** of them are flagged. One flagged picture among three or more never reaches half, so a single
breastfeeding picture still holds only itself. The three numbers are measured constants in
...
| Who may see it? | Any flag from the detectors keeps a picture at family-only viewing, and the distilled `uncovered_person` head is a second opinion that can add one. A picture in a five-minute capture run that is three or more long and at least half flagged is held with the run (`exposure_chain`). A `no` from any of it lifts nothing: nothing clears a flag except you, on the pool page |
