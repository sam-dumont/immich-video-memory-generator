...
def test_a_longer_timeout_reuses_the_answer_banked_under_a_shorter_one(tmp_path) -> None:
    """A timeout bounds the wait, not the answer: 2026-09-01's three relaunches at
    120/600/900 s re-read the same 57 tile-sets because the timeout was in the key."""
    from dataclasses import replace
    ...
    short = replace(
        _request(_page()), limits=VisionRequestLimits(max_output_tokens=4000, timeout_seconds=120)
    )
    long = replace(short, limits=VisionRequestLimits(max_output_tokens=4000, timeout_seconds=900))

    # WHY: query_llm is the external transport; the visual cache remains real.
    with patch("immich_memories.analysis.editorial_gateway.query_llm", new=_answer):
        gateway.ask(short)
        reused = gateway.ask(long)
...
def test_a_pass_override_routes_transport_and_trace_to_its_own_model(tmp_path) -> None:
    """The description student answers its pass; every other pass keeps the primary model."""
    from dataclasses import replace
    ...
    async def _answer(_prompt, config, **kwargs):
        asked.append((config.model, config.base_url))
        kwargs["transport_observer"](LLMTransportAttempt(1, "response", 200))
        return '{"rows": []}'

    trace = Trace()
    gateway = VisualEditorialGateway(
        llm_config=LLMConfig(model="teacher", base_url="http://teacher/v1"),
        cache_path=tmp_path / "judgments.db",
        trace=trace,
        pass_llm_overrides={
            "asset-description": LLMConfig(model="student", base_url="http://student/v1")
        },
    )
    ...
    with patch("immich_memories.analysis.editorial_gateway.query_llm", new=_answer):
        gateway.ask(
            replace(_request(_page()), pass_name="asset-description")  # noqa: S106 - pass identity
        )
        gateway.ask(_request(_page()))

    assert asked == [("student", "http://student/v1"), ("teacher", "http://teacher/v1")]
    requests = trace.as_dict()["requests"]
    assert [entry["model"] for entry in requests] == ["student", "teacher"]


def test_the_same_evidence_never_replays_across_routed_models(tmp_path) -> None:
    """Identity carries the routed model: teacher and student bank apart on one cache."""
    from dataclasses import replace
    ...
    async def _answer(_prompt, config, **kwargs):
        ...
        kwargs["transport_observer"](LLMTransportAttempt(1, "response", 200))
        return f'{{"answered_by": "{config.model}"}}'

    cache = tmp_path / "judgments.db"
    request = replace(_request(_page()), pass_name="asset-description")  # noqa: S106 - pass identity
    teacher_gateway = VisualEditorialGateway(
        llm_config=LLMConfig(model="teacher"), cache_path=cache, trace=Trace()
    )
    student_gateway = VisualEditorialGateway(
        llm_config=LLMConfig(model="teacher"),
        cache_path=cache,
        trace=Trace(),
        pass_llm_overrides={"asset-description": LLMConfig(model="student")},
    )

    # WHY: query_llm is the sole external provider transport this gateway may use.
    with patch("immich_memories.analysis.editorial_gateway.query_llm", new=_answer):
        first = teacher_gateway.ask(request)
        second = student_gateway.ask(request)

    assert calls == ["teacher", "student"]
    assert first.raw_text == '{"answered_by": "teacher"}'
    assert second.raw_text == '{"answered_by": "student"}'
