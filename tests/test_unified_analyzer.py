...
class TestTranscriptionStep:
    """Step 4a-half: transcribe the top candidates after visual sorting."""

    def test_transcription_runs_on_the_top_candidates_only(self):
        """Same cost profile as LLM scoring: the top five, not every candidate."""
        from immich_memories.analysis.unified_analyzer import TRANSCRIBE_TOP_N
        from immich_memories.speech.transcription import Transcript

        class StubSpeechAnalysis:
            """# WHY: replaces SpeechAnalysisService, which owns FFmpeg extraction
            and the VAD and whisper models."""
            ...
            def transcribe_segment(self, video_path, start, end):
                ...
                return Transcript(text="bonjour", language="fr", confidence=0.9)

        speech = StubSpeechAnalysis()
        analyzer = UnifiedSegmentAnalyzer(
            scorer=MagicMock(),
            audio_content_config=AudioContentConfig(),
            analysis_config=AnalysisConfig(),
        ...
        segments = [ScoredSegment(start_time=float(i), end_time=float(i) + 3.0) for i in range(8)]

        analyzer._run_transcription(segments, Path("/fake.mov"))
    ...
    def test_declined_transcription_leaves_the_fields_empty(self):
        class SilentSpeechAnalysis:
            """# WHY: replaces SpeechAnalysisService; models the common case where
            the gate declines."""
        ...
        analyzer = UnifiedSegmentAnalyzer(
            ...
            audio_content_config=AudioContentConfig(),
            analysis_config=AnalysisConfig(),
            speech_analysis=SilentSpeechAnalysis(),
        )
