...
def _expression_assets(
    expression: PersonExpression, fetch: Callable[[str], Sequence[Any]]
) -> list[Any]:
    by_id: dict[str, Any] = {}

    def ids(person_id: str) -> set[str]:
        assets = fetch(person_id)
        for asset in assets:
            by_id.setdefault(asset.id, asset)
        return {asset.id for asset in assets}

    selected = expression.evaluate(ids)
    return sorted(
        (by_id[asset_id] for asset_id in selected),
        key=lambda asset: (asset.file_created_at, asset.id),
    )
...
def videos_in_window(
    ...
    *,
    ...
    if person_expression is not None:
        ...
        return _expression_assets(
            person_expression,
            lambda person_id: client.get_videos_for_person_and_date_range(person_id, date_range),
        )
    if len(person_ids) > 1:
        if match == "or":
            return _union_by_id(
...
def photos_in_window(
    ...
    *,
    ...
    if person_expression is not None:
        ...
        return _expression_assets(
            person_expression,
            lambda person_id: client.get_photos_for_date_range(date_range, person_id=person_id),
        )
