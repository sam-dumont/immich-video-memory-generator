...
Default duration: the range curve again, so about 3 minutes 15 seconds for a three-month season. (The web UI card says 2 minutes 15; see [the note below](#one-ui-and-cli-default-still-disagrees).) The hemisphere flag controls which calendar months map to which season:
